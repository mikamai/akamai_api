...
describe AkamaiApi::Eccu::DestroyRequest do
  ...
  describe "#execute" do
    it "calls 'delete' via savon with a message" do
      ...
      expect(AkamaiApi::Eccu.client).to receive(:call).with(:delete, message: 'asd').and_return fake_response
      subject.execute
    end

    it "returns 'true' if response hash reports to be successful" do
      subject.stub request_body: 'example'
      AkamaiApi::Eccu.client.stub call: double(body: { delete_response: { success: true } })
      expect(subject.execute).to be_true
    end

    it "returns 'false' if response hash reports to be unsuccessful" do
      subject.stub request_body: 'example'
      AkamaiApi::Eccu.client.stub call: double(body: { delete_response: { success: false } })
