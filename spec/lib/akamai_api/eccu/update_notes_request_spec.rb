...
describe AkamaiApi::Eccu::UpdateNotesRequest do
  ...
  describe "#execute" do
    it "calls 'set_notes' via savon with a message" do
      ...
      expect(AkamaiApi::Eccu.client).to receive(:call).with(:set_notes, message: 'asd').and_return fake_response
      subject.execute 'foo'
    end

    it "returns 'true' if response hash reports to be successful" do
      subject.stub request_body: 'example'
      AkamaiApi::Eccu.client.stub call: double(body: { set_notes_response: { success: true } })
      expect(subject.execute 'foo').to be_true
    end

    it "returns 'false' if response hash reports to be unsuccessful" do
      subject.stub request_body: 'example'
      AkamaiApi::Eccu.client.stub call: double(body: { set_notes_response: { success: false } })
